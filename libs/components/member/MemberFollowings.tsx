...
const MemberFollowings = (props: MemberFollowingsProps) => {
  ...
  const routerQuery = router.query as { category?: string };
  const category: string = routerQuery.category ?? "properties";
  ...
  useEffect(() => {
    ...
    if (memberId)
      setFollowInquiry({ ...followInquiry, search: { followerId: memberId } });
    else
  ...
  useEffect(() => {
    ...
    if (!followerId || !isValidObjectId(followerId)) {
      return;
    }
